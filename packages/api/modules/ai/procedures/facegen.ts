...
export const faceGen = protectedProcedure
  ...
  .mutation(async ({ input: { imageUrls }, ctx: { user, req } }) => {
    ...
    const { id: faceIdTuneId } = faceIdTuenResponse as unknown as TuneResponse;
    ...
    await Promise.all(
      GUIDE_TEXT_PROMPTS[newGender].map(
        async (guideTextPrompt, indexGuideTextPrompt) => {
          const randomPromptIndex = Math.floor(
            Math.random() * guideTextPrompt.prompts.length
          );
          const prompt = guideTextPrompt.prompts[randomPromptIndex];

          const guideImages = prompt.images ?? [];
          const randomImageIndex = Math.floor(
            Math.random() * guideImages.length
          );
          ...
          const promptPayload: object = {
            ...
            prompt: {
              text:
                newGender === MALE
                  ? `<faceid:${faceIdTuneId}:1.0> ${prompt.text}`
                  : `<faceid:${faceIdTuneId}:1.0> epic ${prompt.text}`,
          ...
          const baseTunegenderBased =
            newGender === MALE ? TUNE_CYBER_4_2 : TUNE_EPIC_REALISM;
          const promptResponse = await fetch(
            `${DOMAIN}/tunes/${baseTunegenderBased}/prompts`,
            promptOptions
          );
