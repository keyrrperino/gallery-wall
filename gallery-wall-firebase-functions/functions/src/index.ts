...
const replaceFrameBackground = (params: {
  userId: string;
  userGifRequestId: string;
  frameNumber: number;
  imageUrl: string;
}) => {
  const { userId, userGifRequestId, frameNumber, imageUrl } = params;
  const removeBackgroundUrl =
    'https://python-functions-665982940607.asia-southeast1.run.app/' +
    `remove-image-background?userId=${userId}&userGifRequestId=${userGifRequestId}&frameNumber=${frameNumber}&imageUrl=${imageUrl}`;
      ...
      try {
        ...
        for (const file of frameFiles) {
          ...
          try {
            ...
            console.error(
              'removeBackground failed with file path:',
              filePath,
              e1
            );
            try {
              ...
              console.error(
                'removeBackground failed with buffer:',
                filePath,
                e2
              );
              continue; // skip this frame
      ...
      (async () => {
        ...
        frameStream.on('data', (chunk: Buffer) => {
          ...
          webps.forEach((buffer) => {
            count++;
            const key = `frames/${userId}/${userGifRequestId}/${count}.webp`;
            try {
              supabase.storage.from(SUPABASE_BUCKET).upload(key, buffer, {
        ...
        ffmpeg(videoUrl as string)
          .outputOptions([
            ...
            'libwebp',
            '-lossless',
            '1', // or remove for lossy, or use "-qscale", "80"
          ])
          .format('image2pipe')
          .on('error', (err) => {
            logger.error('ffmpeg error', err);
  ...
  async (req, res) => {
    cors({ origin: true })(req, res, async () => {
      try {
        ...
        removeBackground(imageUrl as string)
          .then((blob) => {
            blob
              .arrayBuffer()
              .then((arrayBuffer) => {
                ...
                sharp(outputBuffer)
                  ...
                  .then((compositedBuffer) => {
                    // update
                    const key = `frames/${userId}/${userGifRequestId}/${frameNumber}-composited.png`;
                    try {
                      supabase.storage
                        .from(SUPABASE_BUCKET)
