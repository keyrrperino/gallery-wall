...
import { onValue, ref, update, ref as dbRef } from "firebase/database";

export const GalleryWall: React.FC = () => {
  const [images, setImages] = useState<{ key: string; url: string; isShowed?: boolean }[]>([]);
  const [queueImages, setQueueImages] = useState<{ key: string; url: string; isShowed?: boolean }[]>([]);
  const [modalImage, setModalImage] = useState<{ key: string; url: string; isShowed?: boolean } | null>(null);
  ...
  // Helper to keep track of previous images for diffing
  const prevImageKeysRef = useRef<Set<string>>(new Set());

  // Subscribe to gallery images in Firebase Realtime Database
  useEffect(() => {
    const galleryRef = ref(database, "gallery");
    const unsubscribe = onValue(galleryRef, (snapshot) => {
      const data = snapshot.val();
      let imageList: { key: string; url: string; isShowed?: boolean }[] = [];
      if (data) {
        imageList = Object.entries(data)
          .map(([key, item]: [string, any]) => ({
            key,
            url: item.imageUrlComposited,
        ...
        const newImages = imageList.filter(img => img.isShowed === false);
        setQueueImages((q) => {
          // Only add images not already in the queue
          const qKeys = new Set(q.map(i => i.key));
          return [...q, ...newImages.filter(img => !qKeys.has(img.key)).reverse()];
        });
      } else {
        setImages([]);
        setQueueImages([]);
  ...
  const openModal = useCallback((img: { key: string; url: string }, idx?: number) => {
    setModalImage(img);
    setModalOpen(true);
    setIsClosing(false);
    setIsOpening(true);
    setTimeout(() => {
      // Find the index in the images array for ref
      const imageIdx = typeof idx === "number"
        ? idx
        : images.findIndex(i => i.key === img.key);
  ...
  // After animation, remove modal and update isShowed
  useEffect(() => {
    if (!isClosing) return;
    const timeout = setTimeout(() => {
      ...
      setQueueImages((q) => {
        if (modalImage) {
          // Update isShowed in Firebase
          const imgRef = dbRef(database, `gallery/${modalImage.key}`);
          update(imgRef, { isShowed: true });
          return q.filter(img => img.key !== modalImage.key);
        }
        return q;
      });
      setModalImage(null);
        ...
        <div className="flex flex-wrap gap-4 w-full justify-center">
          {images.map((img, idx) => (
            <img
              ...
              draggable={false}
              className="cursor-none w-[720px] h-auto rounded-lg object-cover"
              style={{
                transition: "box-shadow 0.2s",
                boxShadow:
                  modalImage && modalImage.url === img.url && modalOpen && !isClosing && !isOpening
                    ? "0 0 0 4px #fff"
                    : undefined,
              }}
            />
