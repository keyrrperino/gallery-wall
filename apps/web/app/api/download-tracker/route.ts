...
async function getSupabaseAdminWithRetry() {
  try {
    ...
    if (isJWTError(error)) {
      console.error('JWT error detected, clearing admin client cache and retrying...');
  ...
  try {
    ...
    if (downloadError || !fileData) {
      ...
      if (isJWTError(downloadError)) {
        return new Response(
          JSON.stringify({
            error: "Authentication error. Please clear your browser cache and cookies, then try again.",
            code: "JWT_ERROR",
